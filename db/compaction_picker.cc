...
public:
    LevelCompactionBuilder(const std::string &cf_name,
                           ...
                           NVMCacheOptions* nvm_cache_options = nullptr)
            : cf_name_(cf_name),
              vstorage_(vstorage),
    ...
    FixedRangeTab* picked_range_rab_;
...
Compaction *LevelCompactionBuilder::PickKeyRangeCompaction() {
    ...
    if (!SetupKeyRangeInitialFiles()) {
        // start_level_ == 0, output_level_ == 1
        return nullptr;
    }
    /*if (vstorage_->get_fix_range_tab()->IsCompactWorking()) {
        // 这里需要标注下KeyRange是否正在进行compact，防止多线程冲突
        return nullptr;
    }*/
    auto nvm_cache = dynamic_cast<FixedRangeChunkBasedNVMWriteCache*>(nvm_cache_options_->nvm_write_cache_);

    if(!nvm_cache->NeedCompaction()) return nullptr;

    CompactionItem citem;
    nvm_cache->GetCompactionData(&citem);
    picked_range_rab_ = citem.pending_compated_range_;
    if(picked_range_rab_ == nullptr){
        return nullptr;
    }
    if (picked_range_rab_->IsCompactWorking()) {
        // 这里需要标注下KeyRange是否正在进行compact，防止多线程冲突
        return nullptr;
    }
    ...
    // ... ...
    Usage tab_usage = picked_range_rab_->RangeUsage();

    InternalKey smallest, largest;        // get from KeyRange
    //vstorage_->get_key_range_boundary(&smallest, &largest);
    if(tab_usage.start() != nullptr){
        //TODO 更高效的Keyrange赋值
        smallest = *tab_usage.start();
    }
    if(tab_usage.end() != nullptr){
        largest = *tab_usage.end();
    }
...
Compaction *LevelCompactionPicker::PickKeyRangeCompaction(
        ...
        VersionStorageInfo *vstorage, LogBuffer *log_buffer) {
    LevelCompactionBuilder builder(cf_name, vstorage, this, log_buffer,
                                   mutable_cf_options, ioptions_, db_options_.nvm_cache_options.get());
    // 对应的 LevelCompactionBuilder::PickCompaction()
    return builder.PickKeyRangeCompaction();
}
